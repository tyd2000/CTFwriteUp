...
> 提交图中店铺全名的拼音（无分隔符和声调、字母全小写）加食物价格（阿拉伯数字）即可，
>
> 例如：ctfshow{zhouzilamian114514}。

下载附件解压缩后得到`lamian.png`。

百度识图找到一个图案一样的碗，并且图片上有大众点评的水印，店铺名为“云峰清真餐厅(南滨河路店)”。

在大众点评里找，看到评论里有店面的招牌，得知拉面的名字为云峰牛肉面。

通过百度搜索得到菜单，其中素面7块，一份牛肉的价格是10块，所以云峰牛肉面的价格是17元。

提交`ctfshow{yunfengniuroumian17}`即可。
